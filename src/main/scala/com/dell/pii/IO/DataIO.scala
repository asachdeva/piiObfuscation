...
abstract class DataIO extends SharedSparkSession {
  ...
  private def setDbfsConfig(dataSource: DataSource): Unit = {}
  ...
  private def setKeyConfig(dataSource: DataSource): Unit = {
    ...
    val keyName =
      dataSource.accountKeyName.getOrElse(throw new java.util.NoSuchElementException("Missing Account Key Name"))
    val storageName =
      dataSource.storageName.getOrElse(throw new java.util.NoSuchElementException("Missing Storage Name"))

    if (dataSource.store == Store.GEN2) {
      spark.conf
        .set(s"fs.azure.account.key.$storageName.dfs.core.windows.net", dbutils.secrets.get(vaultName, keyName))
    } else {
      spark.conf
        .set(s"fs.azure.account.key.$storageName.blob.core.windows.net", dbutils.secrets.get(vaultName, keyName))
    }
  }
  ...
  private def setServicePrincipleConfig(dataSource: DataSource): Unit = {
    val vaultName = dataSource.vaultName.getOrElse(throw new java.util.NoSuchElementException("Missing Vault Name"))
    val clientKey =
      dataSource.clientKeyName.getOrElse(throw new java.util.NoSuchElementException("Missing Client Key Name"))
    val secretKey =
      dataSource.secretKeyName.getOrElse(throw new java.util.NoSuchElementException("Missing Secret Key Name"))
    val tenantKey =
      dataSource.tenantKeyName.getOrElse(throw new java.util.NoSuchElementException("Missing Tenant Key Name"))
    ...
    if (dataSource.store == Store.LAKE) {
      ...
      spark.conf.set(
        s"fs.azure.account.oauth2.client.secret.$storageName.dfs.core.windows.net",
        dbutils.secrets.get(vaultName, secretKey)
      )
      spark.conf.set(
        s"fs.azure.account.oauth2.client.endpoint.$storageName.dfs.core.windows.net",
        s"https://login.microsoftonline.com/${dbutils.secrets.get(vaultName, tenantKey)}/oauth2/token"
      )
      spark.conf.set(s"fs.azure.account.auth.type.$storageName.dfs.core.windows.net", "OAuth")
      spark.conf.set(
        s"fs.azure.account.oauth.provider.type.$storageName.dfs.core.windows.net",
        "org.apache.hadoop.fs.azurebfs.oauth2.ClientCredsTokenProvider"
      )
      spark.conf.set(
        s"fs.azure.account.oauth2.client.id.$storageName.dfs.core.windows.net",
        dbutils.secrets.get(vaultName, clientKey)
      )
    }
