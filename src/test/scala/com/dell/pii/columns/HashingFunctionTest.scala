...
class HashingFunctionTest extends FlatSpec with Matchers with BeforeAndAfter with SharedSparkContext {
  ...
  "sha 1 HashingFunctions" should "add a new column with a different value" in {
    val result = source
      .withColumn(HashingFunctionTest.HASH_COL, HashingFunctions.sha1(col(HashingFunctionTest.NAME_COL)))
      .withColumn(
        "same_value",
        when(col(HashingFunctionTest.NAME_COL) === col(HashingFunctionTest.HASH_COL), lit(true)).otherwise(lit(false))
      )
  ...
  "sha 256 HashingFunctions" should "add a new column with a different value" in {
    val result = source
      .withColumn(HashingFunctionTest.HASH_COL, HashingFunctions.sha2(col(HashingFunctionTest.NAME_COL), 0))
      .withColumn(
        "same_value",
        when(col(HashingFunctionTest.NAME_COL) === col(HashingFunctionTest.HASH_COL), lit(true)).otherwise(lit(false))
      )
  ...
  "md5 HashingFunctions" should "add a new column with a different value" in {
    val result = source
      .withColumn(HashingFunctionTest.HASH_COL, HashingFunctions.md5(col(HashingFunctionTest.NAME_COL)))
      .withColumn(
        "same_value",
        when(col(HashingFunctionTest.NAME_COL) === col(HashingFunctionTest.HASH_COL), lit(true)).otherwise(lit(false))
      )
  ...
  "crc32 HashingFunctions" should "add a new column with a different value" in {
    val result = source
      .withColumn(HashingFunctionTest.HASH_COL, HashingFunctions.crc32(col(HashingFunctionTest.NAME_COL)))
      .withColumn(
        "same_value",
        when(col(HashingFunctionTest.NAME_COL) === col(HashingFunctionTest.HASH_COL), lit(true)).otherwise(lit(false))
      )
  ...
  "murmur Hash 3 HashingFunctions" should "add a new column with a different value" in {
    val result = source
      .withColumn(HashingFunctionTest.HASH_COL, HashingFunctions.murmurHash3(col(HashingFunctionTest.NAME_COL)))
      .withColumn(
        "same_value",
        when(col(HashingFunctionTest.NAME_COL) === col(HashingFunctionTest.HASH_COL), lit(true)).otherwise(lit(false))
      )
